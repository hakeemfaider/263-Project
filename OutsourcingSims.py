...
cis_for_excel = {}
for i in range(25):
    outsourcing.update({i: 0})
day_types = ['Weekdays', 'Saturday']
# Change type of day here, Weekdays or Saturday
# day_type = 'Weekdays'
# day_type = 'Saturday'
for truck in list(outsourcing.keys()):
    current_totals = []
    for x in range(5):
        total_cost = 0
        for day_type in day_types:
            ...
            # Only go to it if it has demand
            ...
            while run < runs:
                ...
                Had 24 trucks, want to decrease that as much as possible to Mainfreight
                '''
                our_trucks = truck
                our_shifts = our_trucks * 2
                count = 0
                ...
                for routes in new_route_times:
                    ...
                    if count <= our_shifts:
                        ...
                        cost += ceil(new_route_times[routes] / 4) * mainfreight_cost
            ...
            if day_type == 'Weekdays':
                total_cost += average_cost * 5 * 52 + our_trucks * 75000
            else:
                total_cost += average_cost * 52
        outsourcing[truck] = round(total_cost)
        current_totals.append(round(total_cost))
        print(total_cost)
    print(f'The 95% CI for {truck} trucks is {sms.DescrStatsW(current_totals).tconfint_mean()}')
    cis[truck] = (sms.DescrStatsW(current_totals).tconfint_mean()[0] + sms.DescrStatsW(current_totals).tconfint_mean()[1]) / 2


for truck in list(outsourcing.keys()):
    print(f'{truck} regular trucks with the rest outsourced resulted in a cost of ${outsourcing[truck]}')

m, b = np.polyfit(list(cis.keys()), list(cis.values()), 1)
m = int(m)
b = int(b)
print("M =" + str(m))
print("B =" + str(b))
...
y_values = []
for i in list(cis.keys()):
    y_values.append(m*i + b)

plt.plot(list(cis.keys()), list(cis.values()), 'o')
plt.plot(list(cis.keys()), y_values)
plt.show()
